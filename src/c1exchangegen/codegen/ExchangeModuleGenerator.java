...
public class ExchangeModuleGenerator {
    ...
    private void defineUtilProcs(Module.ModuleBuilder mod) {
        ...
        mod.def(
                proc("_ИнспектироватьИСообщить", args("Об"), false, true,
                        ppIfThen("Клиент",
                                block(
                                        _say("Строка(Об)"),
                                        _say("Строка(ТипЗнч(Об))"),
                                        ifThen("ТипЗнч(Об) = Тип(\"Структура\")",
                                                forEach("Стрк", "Об",
                                                        _say("Строка(Стрк.Ключ) "
                                                                + "+ \" -> \" + Строка(Стрк.Значение) "
                                                                + "+ \": \" + Строка(ТипЗнч(Стрк.Значение))")
                                                )
                                        ),
                                        ifThen("ТипЗнч(Об) = Тип(\"Массив\")",
                                                forEach("Стрк", "Об",
                                                        _say("\"[] \" + Строка(Стрк) "
                                                                + "+ \": \" + Строка(ТипЗнч(Стрк))")
                                                )
                                        ),
                                        ifThen("ТипЗнч(Об) = Тип(\"СписокЗначений\")",
                                                forEach("Стрк", "Об",
                                                        _say(" \" -> \" + Строка(Стрк.Значение) "
                                                                + "+ \": \" + Строка(ТипЗнч(Стрк.Значение))")
                                                )
                                        )
                                ).produce()
        ...
        mod.def(
                proc("СериализоватьМассивСтруктур", args("ЗаписьJSON", "Мас"), true, false,
                        ...
                        forEach("Эл", "Мас",
                                ...
                                block("_ИнспектироватьИСообщить(Эл);"),
                                forEach("ЭлСтр", "Эл",
                ...
                proc("ДесереализоватьМассивСтруктур", args("ЧтениеJSON"), true, true,
                        ...
                        whileLoop("ЧтениеJSON.Прочитать()",
                                ...
                                ifThen("ЧтениеJSON.ТипТекущегоЗначения = ТипЗначенияJSON.КонецОбъекта",
                                        ...
                                        block("_ИнспектироватьИСообщить(Структ);"),
                                        block(
                ...
                proc("СоздатьОбъектыИзМассиваСтруктур", args("Мас"), true, true,
                        ...
                        forEach("Струк", "Мас",
                                ppIfThen("Клиент",
                                        ...
                                        "_ИнспектироватьИСообщить(Струк);",
                                ...
                                ifThen("Имена.Количество() > 2",
                                        ifThen("Имена[1] = \"ТЧ\"",
                                                block(
                                                        "ОбъектДок = Объект;",
                                                        "_ТЧ = Объект[Струк._ИД];"
                                                ),
                                                ifThenElse("_ТЧ.Количество()-1 >= Число(Струк._Н)",
                                                        block("Объект = _ТЧ[Число(Струк._Н)];"),
                                                        whileLoop("_ТЧ.Количество()-1 < Число(Струк._Н)",
                                                                block("Объект = _ТЧ.Добавить();")
                                                        )
                                                )
                                        )
                                ),
                                ...
                                tryCatch(
                                        ifThenElse("ОбъектДок <> Неопределено",
                                                block("ОбъектДок.Записать();"),
                                                block("Объект.Записать();")),
                                        ppIfThen("Клиент", "Сообщить(ОписаниеОшибки());")
                                ),
        ...
        mod.def(
                proc("СоздатьСтруктуру_" + name, args("Объект = Неопределено"), true, true,
                        ...
                        ifThen("Объект <> Неопределено",
                                ...
                                block("_ИнспектироватьИСообщить(Объект);"),
                                block(
                        ...
                        ifThen("Объект <> Неопределено",
                                ...
                                forEach("СтрТаб", "Объект." + qualName,
                                        ...
                                        block(
                                                ...
                                                "Стркт._Тип = Строка(ТипЗнч(Объект.Ссылка)) + \".ТЧ." + name + "\";",
                                                "Стркт._ИД = \"" + qualName + "\";",
                                                "Стркт._Н = _ПрЗнч(СтрТаб.НомерСтроки);",
                                                "Стркт._Ссылка = СериализоватьСсылку(Объект.Ссылка);",
                                                "Стркт._Имя = \"" + name + "\";",
                                                "_Рез.Добавить(Стркт);"
                                        )
                                )
        ...
        mod.def(
                proc("ВыгрузитьВСтруктуры_" + name, args("Отбор = Неопределено"), true, true,
                        ...
                        whileLoop("Выборка.Следующий()",
                                ...
                                block("_ИнспектироватьИСообщить(Выборка.Ссылка);"),
        ...
        if (subch instanceof MappingNode) {
            ...
            if (subchild.getState() == NodeStateContainer.NodeState.Good
                ...
                if (subchild.getInObject().getObjClass() == MetaObjectClass.TabularSection) {
                    ...
                    tblStruct.put("_Тип", null);
                    tblStruct.put("_ИД", null);
